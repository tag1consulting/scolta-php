...
### Fixed
- **`AmazeeClient::provisionTrial()` and `signIn()` now parse the nested API response format.** The Amazee.ai `/auth/generate-trial-access` endpoint now returns credentials nested under a `key` object, and `signIn` wraps the access token under a `token` object. Both methods now check for the nested format first and fall back to the legacy flat format for backwards compatibility.
