...
### Fixed
- **Pagefind filter index CBOR structure: dimensions are now serialized as flat alternating `[name, values, name, values, ...]` instead of nested `[[name, values], [name, values]]`.** The nested structure caused `load_filter_chunk()` to return a null WASM pointer, breaking search on sites with `auto_language_filter` enabled and multilingual content.
