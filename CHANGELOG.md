...
### Fixed
- **Pagefind filter index CBOR structure: both dimension pairs AND value entries within each dimension are now serialized as flat alternating arrays.** The outer structure was `[[name, values], ...]` instead of `[name, values, ...]`, and the inner value entries were `[[valueName, [pages]], ...]` instead of `[valueName, [pages], ...]`. The outer nesting caused `load_filter_chunk()` to return a null WASM pointer. The inner nesting caused all filter-based searches to return 0 results. Both levels are now flat alternating, matching the structure Pagefind's WASM expects.
