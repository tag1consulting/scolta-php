...
### Fixed
- **Language facets now appear when using the Pagefind binary indexer.** `initPagefind()` only loaded the page's primary language index; each per-language Pagefind index only contains its own language's pages, so `search.filters.language` had a single value and `renderFilters` suppressed it. `initPagefind()` now reads `pagefind-entry.json` after init and calls `pagefind.mergeIndex(basePath, { language: lang })` for every other language, so all language instances are searched simultaneously and `search.filters.language` carries counts for every indexed language.
