...
The stemmer parity tests compare the vendored Snowball backend
(`src/Index/Snowball/`) against golden stems generated by `pagefind_stem`
1.0.0 — the exact crate inside Pagefind 1.5.0's query WASM (see
`tests/fixtures/stemmer-corpus/PROVENANCE.md`). All 14 supported languages
are **byte-exact: 0 divergences across 589,069 corpus words** (ca, da, de,
en, es, fi, fr, it, nl, no, pt, ro, ru, sv). The historical sub-100% rates in
earlier revisions of this table were artifacts of the old wamania/php-stemmer
backend, which implemented pre-3.0 Snowball algorithms.
