...
# Generate Pagefind reference fixtures from the test corpus.
...
# Run Pagefind against the corpus
"${PAGEFIND_BIN}" \
    --site "${CORPUS_DIR}" \
...
cat > "${REFERENCE_DIR}/reference-metadata.json" <<METADATA
{
    ...
    "generator_script": "scripts/generate-concordance-fixtures.sh"
}
METADATA

echo ""
echo "Reference fixtures generated at: tests/fixtures/concordance/reference/"
echo "Pagefind version: ${PAGEFIND_VERSION}"
echo "Files:"
find "${REFERENCE_DIR}" -type f | sort | while read -r f; do echo "  $(basename "$f")"; done
echo ""
echo "Next steps:"
echo "  1. git add tests/fixtures/concordance/reference/"
echo "  2. vendor/bin/phpunit tests/Concordance/"
