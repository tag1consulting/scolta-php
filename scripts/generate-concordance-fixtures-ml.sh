...
# Generate Pagefind reference fixtures from the multilingual test corpus.
# Run manually, commit output. Not run during CI.
#
# Handles 19 languages × 5 pages = 95 HTML files:
#   ar (Arabic), zh (Chinese Simplified), da (Danish), nl (Dutch),
#   en (English), fi (Finnish), fr (French), de (German), hu (Hungarian),
#   it (Italian), ja (Japanese), ko (Korean), no (Norwegian), pt (Portuguese),
#   ro (Romanian), ru (Russian), es (Spanish), sv (Swedish), tr (Turkish)
#
# Usage: ./scripts/generate-concordance-fixtures-ml.sh [pagefind-version]
# Example: ./scripts/generate-concordance-fixtures-ml.sh 1.5.1

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
PACKAGE_DIR="$(dirname "$SCRIPT_DIR")"
CORPUS_DIR="${PACKAGE_DIR}/tests/fixtures/concordance/corpus-ml"
REFERENCE_DIR="${PACKAGE_DIR}/tests/fixtures/concordance/reference-ml"
...
# Run Pagefind against the multilingual corpus
"${PAGEFIND_BIN}" \
    --site "${CORPUS_DIR}" \
...
cat > "${REFERENCE_DIR}/reference-metadata.json" <<METADATA
{
    ...
    "generator_script": "scripts/generate-concordance-fixtures-ml.sh"
}
METADATA

echo ""
echo "ML reference fixtures generated at: tests/fixtures/concordance/reference-ml/"
echo "Pagefind version: ${PAGEFIND_VERSION}"
echo "Files:"
find "${REFERENCE_DIR}" -type f | sort | while read -r f; do echo "  $(basename "$f")"; done
echo ""
echo "Next steps:"
echo "  1. git add tests/fixtures/concordance/reference-ml/"
echo "  2. vendor/bin/phpunit tests/Concordance/MultilingualReferenceComparisonTest.php"
