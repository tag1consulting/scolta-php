...
# Generate Pagefind reference fixtures from the extended Wikipedia test corpus.
# Run manually, commit output. Not run during CI.
#
# Processes 19 languages × 5 pages = 95 Wikipedia HTML files (different topics).
#
# Usage: ./scripts/generate-concordance-fixtures-wiki-extended.sh [pagefind-version]
# Example: ./scripts/generate-concordance-fixtures-wiki-extended.sh 1.5.1

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
PACKAGE_DIR="$(dirname "$SCRIPT_DIR")"
CORPUS_DIR="${PACKAGE_DIR}/tests/fixtures/concordance/corpus-wiki-extended"
REFERENCE_DIR="${PACKAGE_DIR}/tests/fixtures/concordance/reference-wiki-extended"
PAGEFIND_VERSION="${1:-1.5.0}"

if [ ! -d "${CORPUS_DIR}" ] || [ -z "$(ls -A "${CORPUS_DIR}"/*.html 2>/dev/null)" ]; then
    echo "Extended Wikipedia corpus not found. Run first:"
    echo "  php scripts/fetch-wikipedia-corpus.php \\"
    echo "    --seed-file=scripts/wikipedia-seed-articles-extended.json \\"
    echo "    --output-dir=tests/fixtures/concordance/corpus-wiki-extended"
    exit 1
fi
...
# Run Pagefind against the extended Wikipedia corpus
"${PAGEFIND_BIN}" \
    --site "${CORPUS_DIR}" \
...
cat > "${REFERENCE_DIR}/reference-metadata.json" <<METADATA
{
    ...
    "generator_script": "scripts/generate-concordance-fixtures-wiki-extended.sh"
}
METADATA

echo ""
echo "Extended Wikipedia reference fixtures generated at: tests/fixtures/concordance/reference-wiki-extended/"
echo "Pagefind version: ${PAGEFIND_VERSION}"
echo "Files:"
find "${REFERENCE_DIR}" -type f | sort | while read -r f; do echo "  $(basename "$f")"; done
echo ""
echo "Next steps:"
echo "  1. git add tests/fixtures/concordance/reference-wiki-extended/"
echo "  2. vendor/bin/phpunit tests/Concordance/ExtendedWikipediaConcordanceTest.php"
